        ...
        *x;
        ...
        for ( f2 = 0 ; f2 < p ; f2++ ) {
          ...
          for ( i = 0 ; i < p ; i++ ) {
                ...
                emap[i] = zmod(f2*x[2]+f3*x[3]+f4*x[4]+f5*x[5]+f6*x[6]+x[7],p);
                edmap[i] = zmod(df1*x[1]+df2*x[2]+df3*x[3]+df4*x[4]+df5*x[5],p);
            }
...
#pragma omp critical(min)
        ...
        }}}} // end of f2, f3, f4, f6 loops (f5 is thread number and f7=1)
    }
