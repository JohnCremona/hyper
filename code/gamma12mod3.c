...
int main (int argc, char *argv[])
{
    ...
    if ( argc < 1 ) { puts ("gamma12mod3 or gamma12mod3 1"); return 0; }
    const int p = 3;
    ...
    {
      /* register int f10; */
      register int f0, f1, f2, f3, f4, f5, f6, f7, f8, f9, f11, // coeffs of f
        df10, df9, df8, df7, df6, df5, df4, df3, df2, df1,           // coeffs of f'
        h0, h1, h2, h3, h4, h5,                                      // potential coeffs of sqrt(f)
        h1h1, h1h2, h1h3, h1h4, h1h5, h2h2, h2h3, h2h4, h2h5, h3h3, h3h4, h3h5, h4h4, h4h5,
        f5s, f4s, f3s, f2s, f1s, f0s,
        i, ny, cnt, ucnt,
        *x;
        int emap[MAXP], edmap[MAXP];

        //        f10 = omp_get_thread_num();
        df9 = f10; //zmod(10*f10, p);
        for ( f11 = 0 ; f11 < 2 ; f11++ ) { df10 = -f11; //zmod(11*f11, p);
        h5 = zmod(half*f11, p);
        h4 = zmod(half*(f10-h5*h5), p);
        h4h4 = zmod(h4*h4, p);
        h4h5 = zmod(h4*h5, p);
        for ( f9 = 0 ; f9 < p ; f9++ ) { df8 = 0; //zmod(9*f9, p);
        h3 = zmod(half*f9-h4h5, p);
        h3h3 = zmod(h3*h3, p);
        h3h4 = zmod(h3*h4, p);
        h3h5 = zmod(h3*h5, p);
        for ( f8 = 0 ; f8 < p ; f8++ ) { df7 = -f8; //zmod(8*f8, p);
          h2 = zmod(half*(f8-h4h4)-h3h5,p);
          h2h2 = zmod(h2*h2, p);
          h2h3 = zmod(h2*h3, p);
          h2h4 = zmod(h2*h4, p);
          h2h5 = zmod(h2*h5, p);
        for ( f7 = 0 ; f7 < p ; f7++ ) { df6 = f7; //zmod(7*f7, p);
          h1 = zmod(half*f7-h3h4-h2h5,p);
          h1h5 = zmod(h1*h5, p);
          h1h4 = zmod(h1*h4, p);
          h1h3 = zmod(h1*h3, p);
          h1h2 = zmod(h1*h2, p);
          h1h1 = zmod(h1*h1, p);
        for ( f6 = 0 ; f6 < p ; f6++ ) { df5 = 0; //zmod(6*f6, p);
          h0 = zmod(half*(f6-h3h3)-h2h4-h1h5,p);
          f5s = zmod(2*(h0*h5+h1h4+h2h3),p);
          f4s = zmod(2*(h0*h4+h1h3)+h2h2,p);
          f3s = zmod(2*(h0*h3+h1h2),p);
          f2s = zmod(2*h0*h2+h1h1,p);
          f1s = zmod(2*h0*h1,p);
          f0s = zmod(h0*h0,p);
        for ( f5 = 0 ; f5 < p ; f5++ ) { df4 = -f5; //zmod(5*f5, p);
        for ( f4 = 0 ; f4 < p ; f4++ ) { df3 = f4; //zmod(4*f4, p);
        for ( f3 = 0 ; f3 < p ; f3++ ) { df2 = 0; //zmod(3*f3, p);
        for ( f2 = 0 ; f2 < p ; f2++ ) { df1 = -f2; //zmod(2*f2, p);
          // set emap[i] = f(i)-f1*i-f0 for i in [0,p-1]
          // and edmap[i] = f'(i)-f1
          for ( i = 0 ; i < p ; i++ ) {
                x = xmap + i*MAXD - 1;  // x[j] = i^j
                emap[i] = zmod(f2*x[2]+f3*x[3]+f4*x[4]+f5*x[5]+f6*x[6]+f7*x[7]+f8*x[8]+f9*x[9]+f10*x[10]+f11*x[11]+x[12],p);
                edmap[i] = zmod(df1*x[1]+df3*x[3]+df4*x[4]+df6*x[6]+df7*x[7]+df9*x[9]+df10*x[10],p);
            }
