...
def phi_term(phi, A_or_P, v=None):
    ...
    """
    #print("computing phi-term for {} ({} case)".format(phi, A_or_P))
    lam = lambda_A(phi) if A_or_P=="affine" else lambda_P(phi)
    #print("lambda factor = {}".format(lam))
    #print("Using alpha(j) for j in {}:  {}".format([e for d,e in phi], [alpha(e,v) for d,e in phi]))
    res = lam * prod([alpha(e,v) for d,e in phi])
    #print("phi-term = {}".format(res))
    return res
...
def beta(i, v=None, verbose=False):
    ...
    which one was originally asked for so that if the same one appears
    again, instead of creating an infinite loop, the variable name
    itself is returned (as a polynomial in that variable).  If the
    computed value is a constant value (in Q(p)) it is returned
    directly and stored; if it is a (linear) polynomial in the same
    variable as associated with this function and the same parameters,
    this gives a simple linear equation to be solved so that a
    constant value (in Q(p)) can again be returned and stored;
    ...
    if v==None:
        v=v0
        if verbose: print("Setting variable "+v0)
    else:
        if v==v0:
            if verbose: print("recursion for variable "+v0)
            return PolynomialRing(F,v0).gen()
    if verbose: print("Computing beta({})".format(i))
    b = sum([phi_term(phi,"affine",v) for phi in split_factorizations(i)])
...
def alpha(i, v=None, verbose=False):
    ...
    a = beta(i,v, verbose=verbose) / p^((i*(i-1))//2)
    if verbose: print("alpha({}) = {}".format(i,a))
    try:
        a=F(a)
        if verbose: print("setting alpha({})".format(i))
        alpha_dict[i] = a
...
def rhostar(d, v=None):
    return sum([phi_term(phi,"projective",v) for phi in split_factorizations(d)])
