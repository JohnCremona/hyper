...
print("For small p we need to compute alpha(n,eps;p) and beta(n,eps; p) separately\n since the generic formulas are only valid for sufficiently large p:")
print("For genus 1 and 2 we only need degrees up to 6 and primes up to 13")
...
print("*"*80)
print()
print("Genus 4 densities: generic formula:")
###########################################################################

rho4p = rho(4,pp)
print("{}\n =\n {}".format(rho4p,rho4p.factor()))

print("The first few values (p<40) are computed individually.")
...
print(" - then the rhos")
for p in primes(50):
    print("p={}: {} = {}".format(p,rho(4,p),rho(4,p)*1.0))
