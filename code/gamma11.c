...
int main (int argc, char *argv[])
{
    ...
    if ( argc < 2 ) { puts ("gamma11 p (or gamma11 p 1)"); return 0; }
    p = atoi(argv[1]);
    if ( p <= 2 || p > MAXP || p==11) { printf ("p must be in [3,%d] and not 11\n", MAXP); return 0; }
...
#pragma omp parallel num_threads(p)
    {
      register int f0, f1, f2, f3, f4, f5, f6, f7, f8, f9,
        ...
        *x;
        ...
        df7 = zmod(8*f8, p);
        for ( f9 = 0 ; f9 < 3 ; f9++ ) {
        if ( f9 == 2 ) f9 = u; // f9 ranges over 0,1,u where u is least non-residue
        df8 = zmod(9*f9, p);
        for ( f7 = 0 ; f7 < p ; f7++ ) {
          df6 = zmod(7*f7, p);
        for ( f6 = 0 ; f6 < p ; f6++ ) {
        ...
        for ( f2 = 0 ; f2 < p ; f2++ ) {
          ...
          for ( i = 0 ; i < p ; i++ ) {
                ...
                emap[i] = zmod(f2*x[2]+f3*x[3]+f4*x[4]+f5*x[5]+f6*x[6]+f7*x[7]+f8*x[8]+f9*x[9]+x[11],p);
                edmap[i] = zmod(df1*x[1]+df2*x[2]+df3*x[3]+df4*x[4]+df5*x[5]+df6*x[6]+df7*x[7]+df8*x[8]+11*x[10],p);
            }
...
#pragma omp critical(min)
                        { // critical block, can only be executed by one thread at a time
                          if (cnt==0)
                            ...
                            if (output_polynomials)
                              printf ("%d 1 [1,0,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d]\n", p,f9,f8,f7,f6,f5,f4,f3,f2,f1,f0);
                            }
                          if (ucnt==0)
                            {
                              xnptless1u ++;
                            ...
                            if (output_polynomials)
                              printf ("%d u [1,0,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d]\n", p,f9,f8,f7,f6,f5,f4,f3,f2,f1,f0);
                            }
        ...
        }}}}}}} // end of f2, f3, f4, f5, f6, f7, f9 loops (f8 is thread number, f10=0 and f11=1)
    }
