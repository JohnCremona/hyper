...
int main (int argc, char *argv[])
{
    ...
    p = atoi(argv[1]);
    if ( p <= 2 || p > MAXP || p==7) { printf ("p must be in [2,%d] and not 7\n", MAXP); return 0; }
...
#pragma omp parallel num_threads(p)
    {
      register int f0, f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12,
        df11, df10, df9, df8, df7, df6, df5, df4, df3, df2, df1,
        ...
        *x;
        ...
        for ( f8 = 0 ; f8 < p ; f8++ ) {
          df7 = zmod(8*f8, p);
          h1 = zmod(half*(f8-h4h4)-h3h5,p);
          h1h1 = zmod(h1*h1, p);
          h1h2 = zmod(h1*h2, p);
          h1h3 = zmod(h1*h3, p);
          h1h4 = zmod(h1*h4, p);
          f6s = zmod(h3h3+2*(h2h4+h1*h5),p);
        for ( f7 = 0 ; f7 < p ; f7++ ) {
          df6 = zmod(7*f7, p);
          h0 = zmod(half*f7-h3h4-h2h5,p);
        ...
        for ( f2 = 0 ; f2 < p ; f2++ ) {
          ...
          for ( i = 0 ; i < p ; i++ ) {
                ...
                emap[i] = zmod(f2*x[2]+f3*x[3]+f4*x[4]+f5*x[5]+f6*x[6]+f7*x[7]+f8*x[8]+f9*x[9]+f10*x[10]+f11*x[11]+f12*x[12]+x[14],p);
                edmap[i] = zmod(df1*x[1]+df2*x[2]+df3*x[3]+df4*x[4]+df5*x[5]+df6*x[6]+df7*x[7]+df8*x[8]+df9*x[9]+df10*x[10]+df11*x[11]+14*x[12],p);
            }
