...
int main (int argc, char *argv[])
{
    ...
    #pragma omp parallel num_threads(p)
    {
        ...
        mincnt = 2*p+1;
        f[10] = 1; f[9] = 0;
        f[7] = omp_get_thread_num();
        for ( f[8] = 0 ; f[8] < 3 ; f[8]++ ) {
        if ( f[8] == 2 ) while ( qmap[f[8]] ) f[8]++; // f[6] ranges over 0,1,c where c is least non-residue
        for ( f[6] = 0 ; f[6] < p ; f[6]++ ) {
        for ( f[5] = 0 ; f[5] < p ; f[5]++ ) {
        for ( f[4] = 0 ; f[4] < p ; f[4]++ ) {
        for ( f[3] = 0 ; f[3] < p ; f[3]++ ) {
        for ( f[2] = 0 ; f[2] < p ; f[2]++ ) {
            ...
            for ( i = 0 ; i < p ; i++ ) {
                ...
                emap[i] = zmod(f[2]*x[2]+f[3]*x[3]+f[4]*x[4]+f[5]*x[5]+f[6]*x[6]+f[7]*x[7]+f[8]*x[8]+f[9]*x[9]+f[10]*x[10],p);
            }
            for ( f1 = 0 ; f1 < p ; f1++ ) {
                for ( f0 = (f[1]?0:1) ; f0 < p ; f0++ ) {
                    ...
                    if ( cnt < mincnt ) {
                        ...
                        if ( trivialgcd(f,df,p) ) {
                            ...
                            if ( mincnt < xmincnt ) {
                                xmincnt = mincnt;
                                printf ("%d pts on y^2=x^10+%d*x^8+%d*x^7+%d*x^6+%d*x^5+%d*x^4+%d*x^3+%d*x^2+%d*x+%d\n", xmincnt, f[8],f[7],f[6],f[5],f[4],f[3],f[2],f[1],f[0]);
                            }
                        }
                    }
                }
